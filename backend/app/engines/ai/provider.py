...
class GroqProvider(AIProvider):
    ...
    # The configured default (openai/gpt-oss-*) is a reasoning model. Reasoning
    # tokens are drawn from the completion budget, so a low effort keeps room for
    # the answer within ``max_completion_tokens``. The coach explains and
    # motivates rather than solving hard problems, so light reasoning is enough,
    # and it keeps latency down on the free tier. Groq accepts "low"/"medium"/
    # "high" for these models.
    ...
    async def generate(self, request: ProviderRequest) -> ProviderResponse:
        ...
        payload = {
            ...
            # ``max_completion_tokens`` is the parameter Groq documents for its
            # current (reasoning) models; ``max_tokens`` survives only as a
            # deprecated alias. Reasoning is returned in a separate ``reasoning``
            # field, so the ``content`` read in ``_normalise`` stays the clean
            # final answer rather than a chain of thought.
